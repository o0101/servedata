...
// constants
  const CANON = 'calendar.dosyago.com'; // the canonical host name
...
// semver check
  {
    const version = JSON.parse(
      fs.readFileSync(path.resolve(APP_ROOT, 'package.json')).toString("utf-8")
    ).engines.node;
...
async function X(req, res, next) {
  ...
  try {
    if ( req.path.includes('/action/') || req.path.includes('/selection/') || req.path == '/' ) {
    ...
    console.error({way, path:req.path});
    console.error(e);
    next(e);
  }
}
