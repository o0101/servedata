...
// constants
  ...
  // file location constants
    export const DB_ROOT = process.env.SD_MODE == 'dev' ? path.resolve(APP_ROOT, "dev-db-servedata") : path.resolve(APP_ROOT, "db-servedata");
...
// database helpers and adapters 
  ...
  export async function getSelectionData({selection, id}, req, res, greenlights) {
    const selectionFileName = path.resolve(SELECTIONS, `${selection}.js`); 
    const {select} = await import(selectionFileName);
    const result = select({id}, req, res, greenlights);
    return result;
  }

  export async function displaySelectionData({selection, id}, req, res, greenlights) {
    const selectionFileName = path.resolve(SELECTIONS, `${selection}.js`); 
    const {select,display} = await import(selectionFileName);
    const result = display(select({id}, req, res, greenlights));
    return result;
  }
  ...
  export async function runStoredAction({action, item}, req, res, greenlights) {
    ...
    const result = await Action(item, {_getTable, getItem, newItem, setItem, getSearchResult}, req, res, greenlights);
    return result;
  }
